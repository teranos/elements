...
This package is the glyph runtime: tray, proximity engine, morph transactions, forms, and the canvas interaction layer (drag, resize, meld). It has zero framework dependencies — pure DOM, Web Animations API, and dependency injection via `configureElements()` and `CanvasHost` for host-specific concerns.

## Core pattern

Every glyph renderer follows the same shape: take a `Element`, return a DOM element.

```typescript
import type { Element } from '@qntx/glyphs';

function createMyElement(glyph: Element): HTMLElement {
...
The `Element` interface is the universal input contract. 19 renderers in QNTX follow this pattern. The package owns the type; renderers live in the host.
...
- **ElementUI's I/O is QNTX.** `pluginFetch`, `pluginWebSocket`, `onMeld`, and config persistence belong to the host factory. The DOM building blocks (`createInput`, `createButton`, `createStatusLine`) are package-owned in `ui-primitives.ts`; the host factory delegates to them.
